...
export function WishlistCard({ item }: WishlistCardProps) {
  ...
  const originText = [item.country, item.estate, item.variety]
    .filter(Boolean)
    .join(" · ");
  ...
  return (
    ...
    >
      <div className="flex items-start justify-between gap-2 mb-3">
        ...
        <span
          className={`text-xs px-2 py-0.5 rounded-full font-medium flex-shrink-0 ${priorityClass}`}
        >
        ...
        <div className="flex items-start gap-1.5 mb-2.5">
          ...
          <div className="flex flex-wrap gap-1 min-w-0">
            {item.flavorNotes.slice(0, 3).map((note) => (
              <span
                key={note}
                className="text-xs px-2 py-0.5 rounded-full bg-primary-soft text-primary"
              >
                {note}
              </span>
            ))}
            {item.flavorNotes.length > 3 && (
              <span className="text-xs text-ink-soft pt-0.5">
                +{item.flavorNotes.length - 3}
              </span>
            )}
          </div>
        ...
        <div className="flex items-center justify-end gap-1 flex-shrink-0">
          {item.purchaseUrl.trim() && (
            <button
              ...
              className="h-8 px-2.5 rounded-lg bg-surface-card text-ink-muted flex items-center gap-1.5
            ...
            >
              ...
              <span className="text-xs font-medium">复制链接</span>
            </button>
          )}
          <button
